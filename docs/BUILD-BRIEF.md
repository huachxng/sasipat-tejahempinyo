...
- Vanilla TS only on the client: no React/GSAP/Lenis/Three (the v0.2 3D hero is hand-written WebGL2 under 15 KB gz). Budgets: home JS ≤ 60 KB gz; any single client chunk ≤ 30 KB gz.
...
| hero-gl (v0.2) | `src/scripts/{graph-gl,sound}.ts`, plus its existing hero files; may add `dataset()`-free helpers to `graph-render.ts` |
| research (v0.2) | `src/lib/bis.ts`, `src/pages/research/**`, `src/pages/data/**`, `src/components/BisChart.astro`, `src/scripts/bis-chart.ts`, `src/styles/research.css`, `tests/unit/bis.test.ts`, `tests/e2e/research.spec.ts`; may add `dataset()` to `src/lib/jsonld.ts` |
| openers (v0.2) | `src/scripts/scroll-scene.ts`, `src/styles/scene.css`, `src/components/{AchievementsOpener,ResearchOpener}.astro`, `tests/e2e/openers.spec.ts`; may edit `src/pages/achievements/index.astro` only to mount the opener |
| archery (v0.2) | `src/lib/archery.ts`, `src/components/{ArcheryTarget,ArcheryRecord}.astro`, `tests/unit/archery.test.ts`; may edit `src/schemas/{achievement,common}.ts`, `scripts/check-content.ts` (score rules only), `content/.obsidian/types.json`, `content/_templates/Achievement.md`, `content/README.md`, the six archery entries' frontmatter, `src/pages/achievements/[id].astro` (mount only), `src/lib/resume.ts` (score fallback only) |
